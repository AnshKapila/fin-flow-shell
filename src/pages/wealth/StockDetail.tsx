...
import { useStockPrice, calculateStockGainLoss } from "@/hooks/useStockPrice";
...
export default function StockDetail() {
  ...
  const { data: liveQuote, isLoading: isPriceLoading } = useStockPrice(stockSymbol || undefined);
  ...
  if (!investment) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <button 
          onClick={() => navigate("/wealth/stocks")}
  ...
  // Parse stored data
  const quantity = investment.bank ? parseFloat(investment.bank) : 0;
  const purchasePrice = investment.interest_rate || 0;
  ...
  const liveCurrentValue = quantity > 0 ? quantity * currentPrice : Number(investment.current_value);
  
  const { amount: returnsAmount, percent: returnsPercent } = calculateStockGainLoss(liveCurrentValue, investedAmount);
  ...
  const displaySymbol = stockSymbol?.split('.')[0] || '';
    ...
    <div className="animate-fade-in pb-24">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-foreground">
                {quantity > 0 ? quantity.toLocaleString() : "N/A"} shares
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-foreground">
                {purchasePrice > 0 ? formatCurrency(purchasePrice) : "N/A"} /share
              </span>
            </div>
              ...
              <div className="text-right">
                ...
                {liveQuote && (
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {isPriceLoading && <span className="text-xs text-muted-foreground">updating...</span>}
                  </div>
                )}
                {liveQuote?.lastUpdated && (
                  <p className="text-xs text-muted-foreground">
                    as of {format(new Date(liveQuote.lastUpdated), "dd MMM, HH:mm")}
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="fixed bottom-20 left-0 right-0 p-4 bg-background border-t border-border">
        <div className="max-w-md mx-auto">
          <Button 
            className="w-full"
            onClick={() => navigate(`/wealth/stocks/${id}/edit`)}
          >
            Edit Investment
          </Button>
        </div>
